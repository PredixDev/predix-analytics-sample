...
public class SQLBuilderForGenericSchema extends SQLBuilderBase implements SQLBuilder {

	public SQLBuilderForGenericSchema(OrchestrationExecutionContext orchestrationExecutionContext, Object queryCriteria, StatementType statementType, List<Object> dataValues, Field field) {
	...
	public SQLBuilderForGenericSchema(Object queryCriteria, StatementType statementType) {
		this.queryCriteria = queryCriteria;
		this.statementType = statementType;
	}

	public String[] buildInsertOrUpdateSql() {
		...
		if (conditions.isEmpty()) {
			...
			for (Object value : dataValues) {
				...
				listValue.add("'" + getAssetId() + "'");
				listValue.add("'" + this.field.getFieldId() + "'");
				listValue.add("to_timestamp(" + ((List) value).get(0) + "::double precision/1000)");
				listValue.add(((List) value).get(1));
